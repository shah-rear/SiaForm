...
export default function FormCard({
  ...
  const dropdownMenu = useMemo(
    () => [
      { name: 'Open', onClick: () => navigate('/forms/' + id) },
      { name: 'Edit', onClick: () => navigate('/forms/' + id + '?edit=true') },
      { name: 'Copy link URL', onClick: copyURLToClipboard },
      { name: 'Show data', onClick: setShowRespondents },
      { name: 'Delete', onClick: deleteForm },
    ],
    [id, navigate],
  );
  ...
  return (
    <div className="relative space-y-2 rounded bg-white py-3 px-5 text-sm shadow">
      ...
      <div className="flex items-center space-x-1">
        ...
        <span className="text-gray-500">Updated {new Date(date).toLocaleString('en-US')}</span>
      </div>
      <Menu>
